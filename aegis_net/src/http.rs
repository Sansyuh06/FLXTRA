...
use aegis_core::{AegisError, HttpMethod, Origin, ResourceType, Result};
use bytes::Bytes;
use reqwest::{
...
impl HttpClient {
    ...
    pub fn new(allow_http: bool, timeout_secs: u64) -> Result<Self> {
        ...
        let client = Client::builder()
            ...
            .build()
            .map_err(|e| AegisError::Network(e.to_string()))?;
    ...
    /// Map reqwest error to AegisError
    fn map_request_error(&self, error: reqwest::Error) -> AegisError {
        if error.is_timeout() {
            AegisError::Timeout(error.to_string())
        } else if error.is_connect() {
            AegisError::ConnectionRefused(error.to_string())
            ...
            if let Some(status) = error.status() {
                ...
                AegisError::Network(error.to_string())
            }
        } else {
            AegisError::Network(error.to_string())
        }
