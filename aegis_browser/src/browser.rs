...
use aegis_core::{BrowserConfig, ContainerId, NavigationRequest, TabId, Result, AegisError};
use aegis_filter::FilterEngine;
use aegis_mcp::McpClient;
use aegis_net::NetworkClient;
use aegis_sandbox::container::ContainerManager;
use aegis_ui::BrowserWindow;
...
impl Browser {
    pub async fn new(config: BrowserConfig) -> Result<Self> {
        info!("Initializing Aegis Browser");
    ...
    pub async fn navigate(&mut self, url: &str) -> Result<()> {
        ...
        let tab = self.tabs.get_mut(&tab_id).ok_or(AegisError::NotFound("Tab".into()))?;
